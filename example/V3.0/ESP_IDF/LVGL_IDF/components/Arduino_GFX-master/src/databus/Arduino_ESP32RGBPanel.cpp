...
uint16_t *Arduino_ESP32RGBPanel::getFrameBuffer(int16_t w, int16_t h)
{
  ...
  esp_err_t err = esp_lcd_new_rgb_panel(&panel_config, &_panel_handle);
  if (err != ESP_OK) {
    printf("ERROR: esp_lcd_new_rgb_panel failed: %d\r\n", err);
    return nullptr;
  }

  err = esp_lcd_panel_reset(_panel_handle);
  if (err != ESP_OK) {
    printf("ERROR: esp_lcd_panel_reset failed: %d\r\n", err);
    return nullptr;
  }

  err = esp_lcd_panel_init(_panel_handle);
  if (err != ESP_OK) {
    printf("ERROR: esp_lcd_panel_init failed: %d\r\n", err);
    return nullptr;
  }

  void *frame_buffer = nullptr;
  err = esp_lcd_rgb_panel_get_frame_buffer(_panel_handle, 1, &frame_buffer);
  if (err != ESP_OK) {
    printf("ERROR: esp_lcd_rgb_panel_get_frame_buffer failed: %d\r\n", err);
    return nullptr;
  }

  if (!frame_buffer) {
    printf("ERROR: get frame buffer failed!\r\n");
    return nullptr;
  }

  _framebuffer0 = (uint16_t *)frame_buffer;
  _framebuffer1 = nullptr;

  return _framebuffer0;
}
